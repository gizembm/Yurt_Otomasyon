...
{
    public partial class formAdminYetkilendirme : Form
    {
        ...
        private void dgvYetkilendirmeListesi_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            // Eğer herhangi bir hücreye tıklanmışsa
            if (e.RowIndex >= 0)
            {
                // Seçilen satırdaki hücrelerden verileri al
                DataGridViewRow row = dgvYetkilendirmeListesi.Rows[e.RowIndex];

                // TextBox'lara veri aktar
                txtKullanıcıId.Text = row.Cells["kullaniciID"].Value.ToString();
                txtKullanıcıAd.Text = row.Cells["kullaniciAd"].Value.ToString();
